...
def main_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔄 Обновить базу", callback_data="upd")],
...
async def on_start(msg: types.Message):
    await msg.answer("🎮 <b>Majestic RP Forum Parser</b>\n\nВыберите:", parse_mode="HTML", reply_markup=main_kb())


@dp.callback_query(F.data == "back")
async def go_back(cb: CallbackQuery, state: FSMContext):
    await state.clear()
    await cb.message.edit_text("🎮 <b>Majestic RP Forum Parser</b>\n\nВыберите:", parse_mode="HTML", reply_markup=main_kb())
...
async def on_upd(cb: CallbackQuery):
    if scraper.busy:
        ...
        return
    await cb.message.edit_text("🔄 <b>Обновление</b>\n\nВыберите сервер:", parse_mode="HTML", reply_markup=upd_kb())
...
async def do_update(cb: CallbackQuery):
    ...
    try:
        if target == "all":
            ...
            cnt = scraper.done
            scraper.busy = False
        await status.edit_text(f"✅ <b>Готово!</b>\n\nЗагружено: {cnt}", parse_mode="HTML", reply_markup=back_kb())
    except Exception as e:
        scraper.busy = False
...
async def show_res(msg: types.Message, data, q, key):
    ...
    for i, r in enumerate(data[:10], 1):
        ...
        if r.get('author_nick'):
            txt += f"   👤 {r['author_nick']}"
            if r.get('author_static'): txt += f" ({r['author_static']})"
            txt += "\n"
        if r.get('violator_nick'):
            txt += f"   ⚠️ {r['violator_nick']}"
            if r.get('violator_static'): txt += f" ({r['violator_static']})"
            txt += "\n"
